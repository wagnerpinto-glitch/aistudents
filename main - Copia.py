...
def simulate_veredai_impact(model, preprocessor, base_student_data):
    """
    ...
    X_before = torch.tensor(preprocessor.transform(df_before).astype(np.float32))
    X_after = torch.tensor(preprocessor.transform(df_after).astype(np.float32))
    
    with torch.no_grad():
        pred_before = model(X_before).item()
        pred_after = model(X_after).item()
...
if __name__ == "__main__":
    ...
    # 1. Pré-processar
    X, y, preprocessor, feature_names = preprocess_data(dataset_path)
    ...
    simulate_veredai_impact(model, preprocessor, exemplo_estudante)